 ...
 * @since 1.0.0
 */
class ImagePro extends \Elementor\Widget_Base {
    ...
    public function __construct( $data = [], $args = null ) {
        ...
        wp_register_script( 'black-widgets-image', BLACK_WIDGETS_PLUGIN_URL . 'assets/js/image.js', [ 'jquery', 'GSAP', 'GSAP-ScrollTrigger', 'black-widgets-simple-parallax' ], BLACK_WIDGETS_VERSION );
    }
	...
	public function get_title() {
		return __( 'Black Image Pro', 'black-widgets' );
	}
	...
	protected function register_controls() {
		...
		if( isset($gsap_options) && !empty($gsap_options) ) {
			...
			$this->add_control(
				'image_movement2',
				[
					'label' 		=> esc_html__( 'Image Movement Animate → From', 'black-widgets' ),
			...
			$this->add_control(
				'image_movement',
				[
					'label' 		=> esc_html__( 'Image Movement Animate → To', 'black-widgets' ),
					'type' 			=> \Elementor\Controls_Manager::SWITCHER,
					'label_on' 		=> esc_html__( 'Enable', 'black-widgets' ),
					'label_off' 	=> esc_html__( 'Disable', 'black-widgets' ),
					'return_value' 	=> 'on',
		...
		$this->add_control(
			'image_parllax',
			[
				'label' 		=> esc_html__( 'Extra Image Parallax', 'black-widgets' ),
				'type' 			=> \Elementor\Controls_Manager::SWITCHER,
				'label_on' 		=> esc_html__( 'Enable', 'black-widgets' ),
				'label_off' 	=> esc_html__( 'Disable', 'black-widgets' ),
		...
		$this->start_controls_section(
			'style_section',
			[
				'label' => esc_html__( 'Image Box Style', 'black-widgets' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Blur', 'black-widgets' ),
				'description' => esc_html__( 'Background /color/ with low opacity is required.', 'black-widgets' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%' ],
				'range' => [
					'px' => [
		...
		$this->add_control(
			'hover_animation',
			[
				'label' => esc_html__( 'Hover Animation', 'black-widgets' ),
	...
	protected function render() {
        ...
        // Render
        echo '<div class="bw-image" style="text-align: ' . $alignment . ';">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

            echo '<div class="bw-img-' . esc_attr( $type ) . ' ' . esc_attr( $settings['hover_animation'] ) . ' bw-cursor-' . esc_attr( $cursor ) .'" id="'. $data_id .'">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            ...
            if ( isset($image_link) && $image_link == 'yes') { echo '<a href="' . esc_url( $settings['image_link_url']['url'] ) . '"' . $target . $nofollow . ' class="bw-image-link">'; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				echo '<img src="' . Group_Control_Image_Size::get_attachment_image_src( $settings['image']['id'], 'thumbnail', $settings ) . '" class=" ' . esc_attr( $parallax ) . ' bw-img-tag bw-cursor-' . esc_attr( $cursor ) .'">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
class_alias('Modernaweb\BlackWidgets\Widgets\ImagePro', 'Black_Widgets\BLACK_WIDGETS_Image_Pro');
