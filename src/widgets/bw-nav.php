 ...
 * @since 1.0.0
 */
class FlatNav extends \Elementor\Widget_Base {

    public function __construct( $data = [], $args = null ) {
        parent::__construct( $data, $args );
        wp_register_style( 'black-widgets-nav', BLACK_WIDGETS_PLUGIN_URL . 'assets/css/nav.css', [], BLACK_WIDGETS_VERSION );
    }
	...
	protected function register_controls() {
			...
			$this->add_control(
				...
				[
					...
					'description'		=> sprintf(
						...
						__( 'Go to the <a href="%s" target="_blank">Menus screen</a> to manage your menus; The menu should be flat', 'blackwidgets' ),
			...
			$this->add_control(
				...
				[
					...
					'raw'				=> sprintf(
						...
						__( '<strong>There are no menus in your site.</strong><br>Go to the <a href="%s" target="_blank">Menus screen</a> to create one.', 'blackwidgets' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Control List Style', 'blackwidgets' ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'On', 'blackwidgets' ),
				'label_off' => esc_html__( 'Off', 'blackwidgets' ),
				'return_value' => 'yes',
				'default' => '',
				'description' => esc_html__( 'Off keeps theme list markers. On lets you style or hide them.', 'blackwidgets' ),
			]
		);
		...
		$this->add_control(
			'nav_list_style_type',
			[
				'label' => esc_html__( 'List Style', 'blackwidgets' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'none',
				'options' => [
					'none'    => esc_html__( 'None (no bullets)', 'blackwidgets' ),
					'disc'    => esc_html__( 'Disc', 'blackwidgets' ),
					'circle'  => esc_html__( 'Circle', 'blackwidgets' ),
					'square'  => esc_html__( 'Square', 'blackwidgets' ),
					'decimal' => esc_html__( 'Decimal', 'blackwidgets' ),
				],
				'condition' => [
		...
		$this->add_responsive_control(
			'nav_list_indent',
			[
				'label' => esc_html__( 'List Indent', 'blackwidgets' ),
	...
	protected function render() {
		...
		$befor_nav          = isset($settings['custom_icon_before_nav']['value']) ? $settings['custom_icon_before_nav']['value'] : '';

		if ( is_array( $befor_nav ) ) {
			$icon_url            = isset( $befor_nav['url'] ) ? $befor_nav['url'] : '';
			$bw_icon_svg_code	 = '<img src="'. esc_url( $icon_url ) . '" />'; // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
		} else {
			$bw_icon_svg_code	 = '<i class="'. esc_attr( (string) $befor_nav ) .'"></i>';
		}
			...
			switch ( $custom_nav_styles ) {
				case 'style7':
					$args = [
						...
						'menu_id'       => 'menu-' . esc_attr( $this->get_nav_menu_index() ) . '-' . esc_attr( $this->get_id() ),
...
class_alias('Modernaweb\BlackWidgets\Widgets\FlatNav', 'Black_Widgets\BLACK_WIDGETS_Nav');
class_alias('Modernaweb\BlackWidgets\Widgets\FlatNav', 'Black_Widgets\BLACK_WIDGETS_FlatNav');
