(function ($) {
    ...
    const initBWInteractiveWidget = function ($scope) {
        const $containerBox = $('.bw-interactive-link-box');
        const gsapSettings = $containerBox.length ? JSON.parse($containerBox.attr('data-gsap-settings')) : {};

        const $image = $('.bw-il-hover-image');
        const $wrapper = $('.bw-il-hover-wrapper');
        const $container = $('.bw-il-image-container');
        const $menuItems = $('.bw-il-menu li');
    ...
    // Initialize for frontend and Elementor editor
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/b_gsap_interactive_links.default', initBWInteractiveWidget);
    });
